"""Verify frozen insider-plan pairs against raw minute executions."""
...
from .insider_plan_eval import _loan_tags, _sections
...
def evaluate(source_dir: Path, raw_path: Path, outcome_dir: Path,
    ...
    if (membership.empty or len(raw) != len(membership) * 4
            ...
            or raw.duplicated(["date", "code", "target_notional",
                               "horizon"]).any()):
        raise ValueError("Incomplete original-minute insider-plan repricing")
    stored = pd.concat([
    ...
    if len(rows) != len(raw) or set(rows.candidate) != {EVENT, CONTROL}:
        raise ValueError("Original-minute insider-plan leg is not frozen")
    rows = rows.merge(_loan_tags(membership, source_dir),
                      on=["date", "pair_code"], validate="many_to_one")
    if len(rows) != len(raw):
        raise ValueError("Original-minute insider-plan leg lacks a title tag")
    ...
    if (not np.isfinite(rows.cash_return).all()
        ...
        raise ValueError("Nonfinite original-minute insider-plan cash return")
    report = {"exact_100k_rows": exact,
    ...
    for size in (20_000, 100_000):
        ...
        for horizon in (1, 5):
            ...
            report["results"][str(size)][str(horizon)] = {
                year: _sections(frame, year) for year in ("2024", "2025")
            }
...
def main() -> None:
    ...
    parser.add_argument("--raw", type=Path, default=Path(
        "data/research/insider_buy/repriced.parquet"))
    ...
    parser.add_argument("--report", type=Path, default=Path(
        "data/research/insider_buy/reprice_report.json"))

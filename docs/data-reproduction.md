...
同行业相对尾盘压力仅做输入审计：`PYTHONPATH=src .venv/bin/python -m trade_research.late_sector_pressure_inputs`；严格跨行业对照未过预设门槛，详见[专项记录](input-gates.md)。
...
下跌尾盘的成交时点仅做输入审计：`PYTHONPATH=src .venv/bin/python -m trade_research.late_volume_timing`；严格同日配对未过冻结门槛，见[专项记录](input-gates.md)。
...
同板块重配输入审计运行 `PYTHONPATH=src .venv/bin/python -m trade_research.late_to_open_reversal --match-board`；结果见[同板块记录](late-board-match-plan.md)。本次未通过输入门槛，故不运行收益复价。
单独的主板总体运行 `trade_research.late_to_open_reversal --main-only`，通过输入门槛后用上述 `size_sensitivity` 参数以 `data/research/late_main_only/repricing_signals.parquet` 复价至同目录 `repriced.parquet`，再用 `trade_research.late_to_open_reversal_eval --output data/research/late_main_only` 汇总；命令均以 `PYTHONPATH=src .venv/bin/python -m` 开头。
尾盘 VWAP 偏离仅运行输入审计 `PYTHONPATH=src .venv/bin/python -m trade_research.late_vwap_pressure`；同日可比配对未过固定门槛，不读取收益，见[VWAP 可比性记录](input-gates.md)。
...
尾盘下跌半方差的输入审计运行 `trade_research.late_variance --threads 8 --output-dir data/research/late_semivariance`，随后运行 `trade_research.late_semivariance_input`；匹配上界未过事前门槛，程序不读收益，见[专项记录](input-gates.md)。
分钟收益负自相关代理的输入审计运行 `PYTHONPATH=src .venv/bin/python -m trade_research.minute_autocovariance --threads 4`；四段配对覆盖未达冻结门槛，程序不读成交或收益，见[停止记录](input-gates.md)。

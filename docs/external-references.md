...
| [Qlib 滚动任务生成器](https://github.com/microsoft/qlib/blob/main/qlib/workflow/task/gen.py) | 滑动／扩展训练窗和 `trunc_segments` | 可复用其明确训练、验证、测试边界的设计；多日标签必须按已完成退出时间隔离。默认 `trunc_days=None` 不会自动替用户消除标签重叠。当前固定排序不训练收益模型，后续训练分支再接入这项能力。 |

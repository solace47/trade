...
p=Path('data/research/alpha158_pool_2026');base=pd.read_parquet(p/'base_features.parquet',columns=PREFIX_COLUMNS)
base['half']=base.date.str[:4]+'Q'+((base.date.str[5:7].astype(int)-1)//3+1).astype(str);base['exchange']=base.code.str[:2]
...
stored=pd.read_parquet(p/'features.parquet').set_index(['date','code'])
for number,r in enumerate(samples.itertuples(),1):
 ...
 np.testing.assert_allclose(stored.loc[(r.date,r.code)].to_numpy(),a,atol=1e-12,rtol=0,equal_nan=True)
...
result={'samples':len(samples),'factor_values_checked':158*len(samples),'maximum_difference':max_difference,'samples_sha256':sha(p/'feature_check_samples.parquet'),'checks':checks,'new_holding_results_read':False,'holdout_inputs_read':True}
